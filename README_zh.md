...
```
13:29:42 INF reforge        已加载 13 个插件
13:29:42 INF factorio       Hosting game at IP ADDR:({0.0.0.0:34197})
13:29:42 INF mod_manager    mod_manager 只会提供为 Factorio 2.0.77 构建的 mod
```

来源列直接告诉你这行来自哪里 —— `reforge`、`factorio`，还是某个插件。
此前 Factorio 的输出走的是裸 `print`，控制台上两种格式并列，
而且服务器自己的输出**根本没进** `logs/reforge.log`。

**Factorio 的每一行都是原样输出**：不改写、不加注、不改等级 ——
你看到的和游戏自己的日志、以及你可能发到论坛上的内容完全一致。

只有当 stdout 是愿意接受颜色的终端时才上色，所以管道进 `grep`
或日志收集器时输出是干净的。`NO_COLOR`、`TERM=dumb`、
以及 `config.yml` 里的 `colour: never` 都能关掉它，`colour: always` 强制打开。
写入文件的那个 handler 永远不带颜色。

### 启动检查报告

正因为 Factorio 的行不被改动，所有要说的话都在服务器起来几秒后单独输出：
...
报告会等一小会儿再输出，而不是在 "in game" 标记处立刻触发 ——
因为有些值得报告的行（尤其是 RCON 绑定）是在那之后才打印的。
