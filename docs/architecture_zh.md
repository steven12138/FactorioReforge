...
├── saves/           槽位、备份、回档
├── versions/        跑的是哪个 Factorio 版本，以及怎么换
└── config.py
```
...
这不是性能考虑。像 `!!qb make` 这样的处理函数要等 Factorio 打印
...
**两圈槽位。** 自动备份只挤自动槽位，寻址写作 `a1`、`a2`。
这是整个模型里唯一一处偏离 QBM 的地方，而且不是偏好问题：
一个每半小时跑一次的定时器，一晚上就能把整段历史挤出去，
而被挤掉的那一份，恰好是某人在动手做危险操作之前特意留的
—— 也就是这个功能存在的全部理由。
`saves.auto_slot_protection` 用同样的方式决定那一圈有几个槽位。

**回档**，`!!qb back <槽位>` 然后 `!!qb confirm`：

1. 校验该槽位里是一个有效的 zip
2. 在聊天里逐秒倒计时，期间可以 `!!qb abort`
3. 停服，并等进程真的退出
...
存档格式升级是一扇单向门：新版本一旦写过这个世界，旧版本就打不开它了。
这让"换服务端二进制"变成了一个回档问题而不是下载问题，
`versions/` 也就照这个形状来 —— 预备、备份、切换、验证、不行就退回去。

有两件事是**读出来**的而不是假设出来的，这正是"停服之前就能知道答案"的原因：

* **二进制自己声明它的窗口。** `--version` 会打印 *map input* 和 *map output*
  两个版本，所以某个版本能不能打开某个世界是一道算术题，
  而不是一张会过期的规则表。
* **存档自己声明它的版本**，就在 `level-init.dat` 开头的四个小端 `uint16` 里。

各个版本是并排的独立目录，用一条软链指向当前生效的那个 ——
因为回滚必须在服务器已经停了、甚至网也不通的时候还能用，
而软链翻转是唯一一种不可能翻到一半的切换。世界要软链回每一棵树里，
是因为 Factorio 从可执行文件推算数据路径，
否则它会把存档写进**那个马上就要被回滚掉的版本目录里**。

切换之前的世界会进一个固定的 `pre-upgrade` 槽位。它在各方面都是第二个
`overwrite`，唯一的区别是回档不会消耗它 —— 它必须活得比一次回档更久，
因为它是唯一一份旧版本还打得开的世界。这个槽位也是"降级"这件事之所以
能被表达出来的全部原因：二进制和世界作为一次操作一起退回去，要么一起，要么都不。
...
没有任何翻译目录含 YAML 布尔键，以及没有任何一份重复定义同一个键 ——
YAML 会保留后一个并且一声不吭，这让 `crash_doctor` 的建议那一行
从写出来到现在一直是废的。
