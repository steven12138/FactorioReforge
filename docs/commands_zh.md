...
```
!!help                 索引：核心命令，然后每个插件一行
!!help warp            单个插件：版本、作者、做什么、有哪些命令
!!help ratio           搜索 —— 找到提供 !!ratio 的那个插件
!!help qb              核心主题：备份的每一条子命令
!!FR plugin list       全部插件，带版本和命令
```

`!!help` 和 `!!FR help` 是同一条命令。帮助是一个人在还不知道有哪些命令时会敲的东西，
所以它不该是最长的那一条。

一个参数覆盖这三件事，因为它们本来就是同一个问题的三种问法：
数字是页码，插件 id 是那个插件，其余一律当搜索。

索引**只对玩家分页**。终端和 Telegram 有滚动历史，而游戏内聊天框没有 ——
那是唯一一个「答案太长就会把开头挤没」的地方。
...
| 命令 | 等级 | 作用 |
|---|---|---|
| `!!help` / `!!FR help` | guest | 索引：核心命令，加上每个插件一行 |
| `!!FR help <页码>` | guest | 索引的第 n 页 |
| `!!FR help <插件>` | guest | 单个插件详情 |
| `!!FR help <关键词>` | guest | 按命令名的任意片段搜索 |
