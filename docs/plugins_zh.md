...
**连不上 Telegram 是网络的问题，不是崩溃。** 连接失败会按逐步拉长的间隔重试 ——
30 秒到 10 分钟，之后稳定在 10 分钟 —— 第一次失败警告一次，之后只偶尔提醒一次，
所以在受限链路后面、或者在等代理起来的机器，最终会自己连上，不需要谁去重载插件。
但被 Telegram **拒绝**的 token 不重试：等下去也修不好它，
而且拿一个坏 token 猛敲正是机器人被限流的原因。
其他异常仍然带 traceback 抛出来 —— 那种时候 traceback 才是重点。
...
`!!version check` 问的是 updater 而不是发布 API，因为一次请求同时给出
渠道标记**和**一路回到 0.12.34 的全部 376 个已发布版本。
要往回退的时候，"最新是什么"从来不是你要问的问题 ——
`!!version check 2.0` 会列出整个系列，里面每一条都是 `!!version install` 认的。
...
`!!seen` 报的是真实时间，用的是这个插件自己记的一份账。
Factorio 自己的答案是 `player.last_online`，那是一个 tick，
把 tick 换算成时长量的是**世界**跑了多久：开着 `auto_pause` 的话，
空服根本不走 tick，于是昨晚离开的人，在下一个玩家登录的那一刻会显示成"20 分钟前"。
Lua API 里没有时钟，所以进出服事件在管道这一侧打时间戳，
写进 `config/server_utils/last_seen.json`。
在这个文件存在之前就离开的人仍然回退到游戏内时间，并且会明说是游戏内时间。
...
**烧料的机器不在电网上。** 石炉吃的 90 kW 是**燃料**，
所以标题写的是 `4.32 MW 燃料`，而不是一个会被读成电力需求的数字；
燃料也和矿石一起列进投入：

```
!!ratio iron-plate 15
iron-plate 15/s —— 共 48 台机器，4.32 MW 燃料
  48 台 stone-furnace  iron-plate  15/s
  需要投入：iron-ore 15/s, coal 1.08/s
  产出相当于 1 条 transport-belt（15/s）  —— 最后一条占了 100%，压缩稍有抖动就掉产能
```

燃料消耗是 `energy_watts / (fuel_value × effectivity)` ——
90 kW 配 4 MJ 的煤，每台炉子 0.0225/s。用哪种燃料是一份偏好列表，
但会被机器 prototype 接受的燃料类别筛过：生物仓烧 `nutrients`、
捕获虫巢烧 `food`，所以对它们来说煤不是"默认值"而是错答案。
列表里没有一种被接受时，取**能量最低**的那种可接受燃料 ——
也就是你最可能已经有的那种。需要自己生产的燃料会被列出来，但不会展开它自己的生产链，
因为燃料不是任何配方的原料，压根不在求解器那张矩阵里。

最后一条传送带被占满时会提示。"正好 1 条带"是最坏情况而不是最好情况：
算术上没问题，而它是压缩一抖就最先掉产能的那个。

`!!ratio machine` 可以看到、也可以改每次计算用哪台机器：

```
!!ratio machine                     用了哪些，按机器分组
!!ratio machine use steel-furnace   钉一台
!!ratio machine drop steel-furnace
!!ratio machine auto                改回「这个存档造得出来的最好的那台」
```

默认是**这个存档真正解锁了的最好的那台** —— 对一个还没研究出三级组装机的存档来说，
一份用三级组装机写的方案毫无用处。什么算解锁，取自 force 已启用的配方，
但要排除回收：每台机器都有一条 `X-recycling` 配方、这些配方一开始就是启用的，
而回收一台三级组装机会产出一台二级组装机 ——
不过滤的话，两级都没研究的存档会报成"二级已解锁"，于是每份方案都高了一级。
某一类里一台都没解锁时，答案给的是**你最先能解锁的那台**而不是最快的那台 ——
对一个还在用一级组装机的人说"低温工厂"，属实但没用。
